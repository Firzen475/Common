...
| Z(zombie) | зомби это процесс, который закончил выполнение, но не передал родительскому процессу свой код возвращения. Процесс в этом состоянии игнорирует kill. Родитель получает код, и освобождает структуру ядра, которое относится к процессу. Бывает еще когда родительский умирает раньше дочернего. Процесс становится сиротой. |
  ...
  ### kill
| Сигнал                                | Важность | Описание                                                                                                         |
|:--------------------------------------|:--------:|:-----------------------------------------------------------------------------------------------------------------|
| SIGHUP                                |    2     | Потеря соединения с управляющим терминалом (прерывание связанных программ); <br/> Для демонов переинициализация; |
| SIGINT                                |    1     | Прерывание из терминала (Ctrl + C).                                                                              |
| SIGQUIT                               |    3     | Остановка с сохранением дампа.                                                                                   |
| SIGILL                                |    3     | Недопустимая инструкция.                                                                                         |
| SIGTRAP                               |    2     | Остановка выполнения для трассировки кода.                                                                       |
| SIGABRT                               |    1     | Принудительное прерывание работы с безопасным закрытием потоков.                                                 |
| SIGBUS                                |    3     | Ошибка шины, при обращении к физической памяти.                                                                  |
| SIGFPE                                |    3     | Ошибка математического вычисления.                                                                               |
| SIGKILL                               |    1     | Безусловное завершение процесса. (кроме зомби и init).                                                           |
| SIGUSR1/SIGUSR2<br/>SIGRTMIN/SIGRTMAX |    3     | Пользовательские сигналы для взаимодействия (например потоков).                                                  |
| SIGSEGV                               |    3     | Cтраница памяти потеряна и ОС посылает сигнал процессу, останавливая выполнение.                                 |
| SIGPIPE                               |    3     | Потеряно прямое соединение между программами.                                                                    |
| SIGALRM/SIGVTALRM/SIGPROF             |    3     | Сообщения об истечении времени (alarm()).                                                                        |
| SIGTERM                               |    1     | Запрос на завершение программы (kill посылает по умолчанию).                                                     |
...
| SIGTTIN/SIGTTOU                       |    3     | Терминал отправляет при попытке ввода/вывода.                                                                    |
